-- VIDA OS™ — Gate 002 / M2.3
-- Source: rehearsal migration gate002_v3_m23_active_authorization_auth_delete_block
...
begin
  -- IMPORTANT: do not auto-revoke an active authorization merely because
  -- auth.users is being deleted. For an active row, ON DELETE SET NULL must
  -- hit chk_active_requires_auth_user and block the non-canonical shortcut
  -- (AUTH-L2). Revoked rows may still lose the FK identifiers while keeping
  -- their snapshot labels/history.
