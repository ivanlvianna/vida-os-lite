...
begin
 if vida_internal.current_auth_uid() is not null then raise exception 'activate_client_from_vri é função de provisionamento backend — não deve ser chamada em sessão interativa (auth.uid() = %)',vida_internal.current_auth_uid();end if;
...
alter table public.client_activation_seeds owner to vida_identity_owner;
alter function public.activate_client_from_vri(uuid,text,text,text,uuid,text,text) owner to vida_identity_owner;
