...
/**
 * Reconstructed candidate of the InviteClientWorkflow described by AH-001.
 *
 * Contract preserved from the architecture handbook:
 * - only planner_owner of the target Client Account may invite;
 * - invitation uses Supabase Auth admin inviteUserByEmail();
 * - invitation does not create membership or authorization;
 * - GrantAccessWorkflow remains a separate later use case.
 *
 * This file is a controlled reconstruction because the original historical
 * artifact is not recoverable from the accessible Git history.
 */
